...
export default function VerifyEmail() {
	...
	async function handleSubmit() {
		...
		const res = await fetch(`/api/auth/account-activation?u=${sessionStorage.getItem("cuid")}`, { method: "GET", headers: { "X-Code-Verification": `${code}` } });
		...
		<div className="bg-slate-700 flex items-center justify-center min-h-screen">
			<div className="bg-white p-6 shadow-xl shadow-gray-700 rounded-lg w-96">
				<div className="flex items-center justify-center mb-4">
					<i
						className={`fas fa-check-circle text-xl text-start ${isValid ? "text-green-500" : "text-gray-500"}`}
					></i>
				...
				<p className="text-gray-600 mb-6 text-justify">
					Para garantir a segurança da sua conta, enviamos um código de
					verificação para o seu e-mail. Por favor, acesse a mensagem que
					enviamos e insira o código nos campos abaixo para concluir o processo
					de verificação.
				</p>
				...
				<button
					...
					className="w-full bg-slate-700 text-white py-2 rounded-lg mt-6"
				>
					Confirmar
				</button>
				<link
